...
public class DTNHost implements Comparable<DTNHost> {
	...
	public void forceConnection(DTNHost anotherHost, String interfaceId, boolean up) {
		...
		try {
			transmitSpeed = Integer.parseInt(interfaceId);
		} catch (Exception e) {
			transmitSpeed = 0;
		}
		...
		if (transmitSpeed == 0) {
			...
			ni.setTransmitSpeed(transmitSpeed);
			no.setTransmitSpeed(transmitSpeed);

			assert (ni.getInterfaceType().equals(
					no.getInterfaceType())) : "Interface types do not match.  Please specify interface type explicitly";
		}
